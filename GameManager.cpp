...
void GameManager::run() {
  ...
  sf::FloatRect viewport(0.f, 0.f, windowManager.window.getSize().x,
                         windowManager.window.getSize().y);
  ...
  while (windowManager.isActive()) {
    ...
    // Update viewport position to keep player centered until it reaches the
    // edge
    sf::Vector2f playerPosition = player.getPosition();
    viewport.left = std::clamp(playerPosition.x - viewport.width / 2.f, 0.f,
                               level.getSize().x - viewport.width);
    viewport.top = std::clamp(playerPosition.y - viewport.height / 2.f, 0.f,
                              level.getSize().y - viewport.height);
    ...
    windowManager.drawText(
        "Player Position: (" +
            std::to_string(static_cast<int>(std::round(playerPosition.x))) +
            ", " +
